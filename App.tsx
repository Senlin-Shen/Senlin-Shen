...
const App: React.FC = () => {
  ...
  // 前端图片压缩：限制宽度 1200px，质量 0.7，确保 Base64 不超过 Vercel 传输限制 (约 4.5MB)
  ...
  const startAnalysis = async () => {
    ...
    setIsParsing(true);
    setErrorMessage(null);
    setCurrentReport(""); // 清空旧报告，准备接收流式输出
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      const context = `当前解析报告: ${currentReport}\n历史记录数: ${history.length}\n事件摘要: ${history.slice(-3).map(h => h.display).join('; ')}`;
    ...
    <div className="flex h-screen bg-[#f8fafc] overflow-hidden select-text text-slate-900">
      ...
      <aside className={`${sidebarOpen ? 'w-80' : 'w-0'} transition-all duration-300 bg-[#1e293b] text-white flex flex-col z-20 shadow-xl overflow-hidden`}>
        <div className="p-6 border-b border-slate-700 flex items-center justify-between shrink-0">
          ...
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1 hover:bg-slate-700 rounded-lg"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="p-6 bg-slate-900 border-t border-slate-700">
           <button 
             ...
             className="w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95"
           >
             <PlusCircle className="w-5 h-5" /> 导入新病历
           </button>
           <input 
             type="file" 
             ref={fileInputRef} 
          ...
          <div className="absolute top-20 right-8 left-8 lg:left-auto lg:w-96 z-50 animate-in slide-in-from-top-4 duration-300">
            <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-2xl shadow-xl flex items-start gap-3">
              ...
              <div className="text-sm flex-1">
                <p className="font-bold">临床引擎反馈</p>
                <p className="opacity-90 break-all">{errorMessage}</p>
              </div>
              <button onClick={() => setErrorMessage(null)} className="p-1 hover:bg-red-100 rounded-lg"><X className="w-4 h-4" /></button>
            </div>
          </div>
        )}
        ...
        <header className="h-16 bg-white border-b flex items-center justify-between px-8 shrink-0 z-10 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full border border-emerald-100 font-bold text-[10px] uppercase tracking-widest">
              <ShieldCheck className="w-4 h-4" /> 豆包 Ark 流式临床引擎已就绪
            </div>
          </div>
          <div className="text-xs text-slate-400 font-medium">Project Nexus v2.0</div>
        </header>

        <div className="flex-1 overflow-y-auto p-8 flex flex-col lg:flex-row gap-8 custom-scrollbar">
          <div className="flex-1 space-y-8 max-w-4xl min-w-0">
            {previews.length > 0 && (
              <section className="bg-white p-6 rounded-3xl border-2 border-blue-500/20 shadow-xl shadow-blue-500/5 animate-in fade-in zoom-in-95 duration-300">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-bold text-slate-700 flex items-center gap-2">
                    <Paperclip className="w-4 h-4 text-blue-500" /> 待处理队列 ({previews.length})
                  </h3>
                  <div className="flex gap-2">
                    ...
                    <button 
                      ...
                      disabled={isParsing} 
                      className="px-6 py-2 bg-blue-600 text-white rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-blue-700 disabled:opacity-50 transition-all shadow-md active:scale-95"
                    >
                      {isParsing ? <Loader2 className="w-4 h-4 animate-spin"/> : <Sparkles className="w-4 h-4"/>}
                      {isParsing ? '正在识别...' : '开始结构化分析'}
                    </button>
                  </div>
                </div>
                  ...
                  {previews.map(p => (
                    <div key={p.id} className="relative group">
                      ...
                      <button 
                        ...
                        className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow-lg lg:opacity-0 lg:group-hover:opacity-100 transition-opacity"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </div>
            ...
            <section className="bg-white rounded-3xl border border-slate-200 shadow-sm min-h-[500px] flex flex-col overflow-hidden">
              <div className="p-6 border-b flex items-center justify-between bg-slate-50/50 shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="font-bold text-slate-800">结构化临床档案报告</h2>
                </div>
                {isParsing && (
                  <div className="flex items-center gap-2 text-blue-600">
                    <Loader2 className="w-4 h-4 animate-spin" />
                    <span className="text-xs font-bold uppercase tracking-tighter">AI 正在生成流...</span>
                  </div>
                )}
                {currentReport && !isParsing && (
                  <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 border border-slate-200" title="导出全文">
                    <Download className="w-4 h-4"/>
                  </button>
                )}
              </div>
              <div className="p-10 prose prose-slate max-w-none flex-1 overflow-y-auto">
                {currentReport ? (
                  <div className="whitespace-pre-wrap text-slate-700 leading-relaxed select-text font-medium text-lg">
                    {currentReport}
                  </div>
                ) : (
                  <div className="h-full flex flex-col items-center justify-center text-slate-300 py-32">
                    ...
                    <p className="font-bold text-xs tracking-[0.2em] text-slate-400 uppercase">等待临床资料导入</p>
                    <p className="text-[10px] mt-2 opacity-60">支持多张图片全选上传，自动前端压缩识别</p>
                  </div>
                )}
              </div>
            </section>
          ...
          <div className="w-full lg:w-[420px] shrink-0 h-fit sticky top-8">
            <ChatInterface 
              ...
              isTyping={isChatting} 
            />
            <div className="mt-4 p-4 bg-white/60 rounded-2xl border border-slate-200 backdrop-blur-sm">
              <div className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest">引擎连接状态</div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                <span className="text-[11px] font-bold text-slate-600 tracking-tighter uppercase">Volcengine Ark · Stable</span>
              </div>
            </div>
          </div>
        </div>
